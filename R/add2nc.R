...
#'
#' @description
#' Imports one or more numeric vectors from `source_df` into `nc_path`,
#' aligning source rows to the NetCDF time axis using exact timestamp matching.
#' Optionally shifts source timestamps by a fixed minute offset before matching
#' and creates target NetCDF variables if they do not already exist.
#'
#' @details
#' **Process overview:**
...
#' @param source_time_col Character scalar. Column name in `source_df` containing
#'   timestamps. Default `"TIMESTAMP"`.
#' @param vars_to_add Character vector. Source column names to import from `source_df`.
#' @param new_var_names Character vector or `NULL`. Output variable names in the NetCDF
#'   file. If `NULL`, defaults to `paste0(vars_to_add, "_profile")`.
#' @param units Character scalar or character vector. Units string(s) assigned to newly
#'   created NetCDF variables. If length 1, the same units are used for all variables.
#'   Ignored for variables that already exist in the file.
#' @param longname Character scalar, character vector, or `NULL`. Long name(s) assigned
#'   to newly created NetCDF variables. If length 1, the same long name is used for all
#'   variables. If `NULL`, defaults to
#'   `paste(vars_to_add, "imported from source dataframe")`.
#'   Ignored for variables that already exist in the file.
#' @param missval Numeric scalar. Missing-value code used when building full-length
#'   output vectors. Default `-9999`.
...
#' @param tz Character scalar. Time zone used for NetCDF timestamp conversion and
#'   matching. Default `"UTC"`.
#' @param source_tz Character scalar. Time zone used when parsing source timestamps.
#'   Defaults to `tz`.
#' @param time_offset_minutes Numeric scalar. Fixed minute offset applied to source
#'   timestamps before matching. Default `6`.
#' @param time_offset_direction Character scalar. One of `"subtract"`, `"add"`,
#'   `"none"`. Controls how `time_offset_minutes` is applied to source timestamps.
#'   Default `"subtract"`.
#' @param new_file Logical scalar. If `TRUE`, a new NetCDF file is created for output.
#'   If `FALSE`, the existing file at `nc_path` is modified in place. Default `TRUE`.
#' @param output_nc_path Character scalar or `NULL`. Path for the output NetCDF file.
#'   Required if `new_file` is `TRUE`. If `NULL`, defaults to
#'   `<nc_path>_updated.nc`.
#'
#' @return A [tibble::tibble()] with one row per imported variable containing:
#' \describe{
#'   \item{source_var}{Source column name in `source_df`.}
#'   \item{target_var}{Target variable name written in the NetCDF file.}
#'   \item{matched_rows}{Number of source rows matched to NetCDF timestamps.}
#'   \item{nc_time_points}{Length of the NetCDF time axis.}
#'   \item{resolved_time_name}{NetCDF time axis name that was used.}
#'   \item{time_offset_minutes}{Minute offset value that was applied.}
#'   \item{time_offset_direction}{Offset direction that was applied.}
#'   \item{output_nc_path}{Path to the output NetCDF file.}
#' }
#'
#' @examples
#' \dontrun{
#' result <- add2nc(
#'   nc_path = "my_flux_file.nc",
#'   source_df = profile,
...
#' }
#'
#' @importFrom ncdf4 nc_open nc_close ncvar_get ncatt_get ncvar_def ncvar_add ncvar_put
...
add2nc <- function(
  ...
  if (time_var_exists) {
    nc_time_raw   <- ncdf4::ncvar_get(nc, resolved_time_name)
    nc_time_units <- ncdf4::ncatt_get(nc, resolved_time_name, "units")$value
  } else {
    if (is.null(time_dim)) {
  ...
  nc_time_calendar <- tryCatch(
    ncdf4::ncatt_get(nc, resolved_time_name, "calendar")$value,
  ...
  nc_cf         <- CFtime::CFtime(nc_time_units, calendar = nc_time_calendar, nc_time_raw)
  nc_timestamps <- as.POSIXct(CFtime::as_timestamp(nc_cf, asPOSIX = TRUE), tz = tz)
  ...
  if (length(vars_missing_in_file) > 0) {
    for (i in seq_along(new_var_names)) {
      ...
      source_var <- vars_to_add[[i]]  # nolint: object_usage_linter
  ...
  for (i in seq_along(vars_to_add)) {
    ...
    ncdf4::ncvar_put(nc, target_var, out)
  }
